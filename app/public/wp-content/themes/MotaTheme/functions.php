...
function enqueue_load_more_photos_script()
{
    ...
    // Utilise wp_localize_script pour passer des paramètres à mon script
...
{
    //démarre une session (via session_start//
    session_start();
...
function filter_photos()
{
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'filter_photos') {
        // Récupére les filtres et nettoye-les
        $filter = array_map('sanitize_text_field', $_POST['filter']);
        $page = $_POST['page'];

        // Ajoute des messages de débogage pour voir les valeurs reçues
        error_log('Filter values: ' . print_r($filter, true));

        // Construit la requête WP_Query avec les filtres
        $args = array(
            'post_type'      => 'photo',
            'posts_per_page' => 8,
            'order'          => 'ASC',
        ...
        // Ajoute la taxonomie pour la catégorie 
        ...
        // Ajoute la taxonomie pour l'année si elle est spécifiée
        if (!empty($filter['years'])) {
            $args['order'] = ($filter['years'] == 'date_desc') ? 'DESC' : 'ASC';
        }

        // Ajoute la taxonomie pour le format si elle est spécifiée
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) :
                $query->the_post();
                // Récupére et affichez les informations de chaque photo
                $photoId      = get_post_thumbnail_id();
                $reference    = get_field('reference');
                $refUppercase = strtoupper($reference);

                // Ajoute des messages de débogage pour les champs ACF
            ...
            <?php
            // Réinitialise les données de requête après la boucle de requête
            wp_reset_query();
        } else {
